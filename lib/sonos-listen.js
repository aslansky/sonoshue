...
var Listener = function(device, ownIp) {
  this.device = device;
...
Listener.prototype.addService = function(serviceEndpoint, callback) {
  if (!this.server) {
    ...
    var _ip = this.ownIp || ip.address();
    var opt = {
      url: 'http://' + this.device.host + ':' + this.device.port + serviceEndpoint,
      method: 'SUBSCRIBE',
      headers: {
        callback: '<http://' + _ip + ':' + this.port + '/notify>',
        NT: 'upnp:event',
        Timeout: 'Second-3600'
      }
    };
    ...
    request(opt, function(err, response) {
      if (err || response.statusCode !== 200) {
        callback(err || response.statusMessage || response.statusCode);
      } else {
